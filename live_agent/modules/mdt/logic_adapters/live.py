...
from live_agent.modules.chatbot.logic_adapters.live import AutoAnalysisAdapter
from live_agent.modules.chatbot.constants import ITEM_PREFIX

__all__ = ["IndexedAnalysisAdapter", "IndexedQueryAdapter"]


class BaseIndexedAdapter(object):
...
class IndexedAnalysisAdapter(AutoAnalysisAdapter, BaseIndexedAdapter):
    """
    Analyze a curve on live (inherited from AutoAnalysisAdapter) with the option of
    using an index curve as the time reference
    """
    ...
    def execute_action(self, statement, selected_asset):
        ...
        if has_index_mention:
            result = self.process_indexed_analysis(statement, selected_asset)
        else:
            result = super().execute_action(statement, selected_asset)

        return result


class IndexedQueryAdapter(BaseBayesAdapter, NLPAdapter, WithAssetAdapter, BaseIndexedAdapter):
    """
    ...
    def can_process(self, statement):
        ...
        return can_process and ("ETIM" in statement.text.upper().split(" "))
